...
The final R code can be found in the Proposal_FINAL_Submitted.Rmd file.

## Introduction

We are interested in efficient pricing of flight training courses and factors associated with student success. We plan to analyze costs that Durham Flight Training Centre must incur for offering flight training. We also plan to study combinations of exercises and aircrafts associated with success in training aviation students. We define students as ‘successful’ if they have flown solo at least once during the course of their training.
...
+ Durham Flight Centre’s website also lists the hourly rental rate for each plane. This information will help us estimate revenue and determine the profit margins for the flight centre.
2. Whether the number of flight hours have differed across different times of the year. And if some planes are more in demand during certain months.
+ This will help us determine how demand for flights varies throughout the year. If we find that certain planes are more in demand during certain months, then we can analyse how the flight centre can adjust the prices for training with different aircrafts to increase profit.
3. How much money it costs to train the average student until they can safely take off, land and fly solo.

To answer questions related to training of students, we would like to build a “profile” for students. In order to determine how the centre can maximize student success, we want to know:
...
By further studying the types of exercises and aircraft models suited for those exercises, we can determine how the training centre should schedule certain exercises for students. More student success can help the centre attract more students.

### Data Analysis Plan

1. **Monthly Training Duration:** Grouping the sessions by month and summing up the duration would show us the total time flown per month. This would allow us to see if demand for flight training is influenced by season. 

2. **Profit:** Summing up the training duration and multiplying that by the costs of fuel and instructor salaries would give us the total costs. Multiplying the sum of training duration by the hourly rental rate of each plane would give us the revenue. The profit would then be the revenue minus the costs. We can plot the type of planes vs the total cost for each plane, and obtain insight into which plane(s) are the most profitable.

3. **Costs of Training Average Student:**
To estimate the cost for the “average student” in the training centre, we will group the data by different student IDs and plane types and then find the average number of hours spent in flight training by each student for each plane type. We will then multiply these values with the average costs from above, and create a graph visualizing our findings.

4. **Successful students:**
First, we will see which students flew solo at least once, by filtering for rows where training type contains the substring “solo”. Then we will graph the frequency of exercises completed by each student to see the most frequent exercises completed by “successful” students.
We'll repeat the same process with filtering out students who got to fly “solo”. We will then add up the duration of hours each successful student flew with different types of planes to see if successful students used certain planes more frequently than others. Grouping the most frequent exercises completed by type of airplane will show us if certain aircrafts are better suited for particular exercises.

### Data

Our primary source of data is a dataset provided by Durham Flight Training Centre. This dataset logs the exercises students completed with different instructors on different dates. It specifies durations of the training sessions, whether exercises were completed solo or with an instructor, and which aircrafts were used. The dataset only spans the years 2002, 2015-2020, so we can only observe patterns and compare trends over the span of five years. It may not be suitable for generalizing trends to longer periods of time. We decided there was more reliable data for 2018-2020 than the other years so we focused our analysis on these years. We used the Durham Flight Centre's website: https://durhamflightcentre.com/ to get hourly aircraft rates, flight training package rates, and instructor rates.  

We gathered data on the aircrafts’ ownership costs from aopa.org, the website of the "Aircraft Owners and Pilots Association", and https://cessna150152club.org/, the website of the "Cessna 150-152 Club", a membership club and nonprofit dedicated to educating prospective and current pilots about the Cessna-152 and Cessna-150. We believe these organizations’ numbers should be reliable since these organizations and communities have access to active users of Cessna-172, Cessna-152, Cessna-150 planes. Whenever sites provided a range of prices, we just used the median for our computations as the median is a good indicator of the "middle value" in a distribution and is less likely to be affected by outliers. We used the Tillsonburg airport website (https://www.tillsonburg.ca/en/live-and-play/Fuel.aspx#) to find the cost of Avgas (the same type of gas the training centre's aircraft use). In April 2021, it said Avgas was \$1.69/L. We believed this was an accurate estimate for fuel pricing, since Durham Flight Centre is in the same province as Tillsonburg airport so region and market conditions affecting fuel prices should be similar. See Appendix for more information about our data sources.
...
my_cd %>% select(Aircraft, Duration, Training_Type, Year_Month,  Month) %>%
  ...
  ggtitle("Figure 1: Months vs Duration of Flight Hours") +
  ylab("Total Hours Flown During Month") +
  xlab("Year - Month")
```

Note that data for certain months is missing. 2020 only has data for 7 months because of the lockdown from the pandemic. 2018 has no data for January. Finding the aggregate duration for each month across all years (eg. adding up duration for 2018 January, 2019 January, etc) would yield skewed results. So we decided to treat months for different years separately.

Training duration generally increases during the months of June, July, August, September, and October, usually peaking around August. This is most likely due to the fact that those months tend to be warmer, and invite clearer skies which are an important consideration when flying. We think it'd be interesting to compare the training duration with more detailed weather data on temperatures and precipitation. Moreover, we see that the hours peak around 60 in 2018, around 180 in 2019, and oddly enough, 2020 seems to have the highest peak of over 400 flight hours. So even during a pandemic, the flight center was able to maintain growth and bring in students to log more flight hours. This steady growth in flight hours, which is synonymous to the demand for training, has been increasing over the years, as shown in our graphs. But to help tackle the slump in demand that the flight center sees every winter or spring, the centre could consider decreasing prices for the training courses or give special package rates (of course while staying above the fixed + variable costs), which in turn could possibly increase demand and growth in the future.
...
clean_data_withcosts %>%
  ...
  summarise(total_fuel_consumed_gallons = sum(Fuel_Cost_Gallons_Session)) -> total_aircraft_costs

total_aircraft_costs  %>% mutate(price_total_fuel_CAD = total_fuel_consumed_gallons*(1.69/0.2199692)) %>%
  filter(Aircraft %in% c("C-150", "C-152", "C-172")) -> total_aircraft_costs
(total_aircraft_costs)
```

We found that over the years 2018-2020, the training centre’s Cessna-172 aircrafts consumed around \$114403.51 worth of fuel (14890.68 gallons) in total. The Cessna-152 aircrafts consumed around \$40243.54 worth of fuel (5238.07 gallons) in total. The Cessna-150 aircrafts consumed around \$7197.95 worth of fuel (936.88 gallons) in total. This is consistent with the fact that during 2018-2020, the Cessna-172 was used for training more often, and its hourly fuel cost is more expensive than that of the Cessna-152 and Cessna-150. It is interesting to note that the training centre spent nearly 2 times the amount of time on Cessna-172 than the Cessna-152, but 3 times the amount of fuel. This suggests that the C-172 is a more expensive plane to operate. The flight centre could consider offering more special packages with the C-152 \& C-150 and decreasing the hourly rates for students, with the aim of increasing demand for C-152 \& C-150.  

```{r, echo = FALSE, warning=FALSE, message=FALSE}
fuel = my_cd %>% mutate(Gallons_Fuel_Session = ifelse(Aircraft == "C-172", 8.4*Duration,
                                                             ifelse(Aircraft == "C-152", 6.1*Duration, 
                                                              ifelse(Aircraft == "C-150", 5.6*Duration, 0)))) %>%
...
Another cost incurred by the training centre is that of paying instructors for dual training and ground school. We found the duration of training sessions with “LF-dual” training type and “GROUND” Aircraft type. For sessions involving “GROUND” Aircraft type, we noticed that the duration was entered as NA (not given) so we didn’t know how many hours the instructors were being paid to teach Ground School sessions. We approximated the duration of each ground school session by comparing it to the average durations of other sessions in which students completed the exact same combination of exercises. We multiplied the approximate hours of ground training by the hourly instructor pay rates listed on Durham Flight Centre’s website ($65 (CAD)). See Appendix for more details.
...
We estimated the flight centre’s revenue using the durations of training, and the hourly costs of renting planes listed on the flight centre’s website. We didn’t know how much the flight centre earned from Ground School alone, so we estimated it using the flight centre's package rates. See Appendix for more details on our assumptions. 
...
upper_bound_month_revenue %>%
  ggplot(aes(x = Year_Month, y = total_revenue_per_month, fill = Aircraft)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Year_Month vs Monthly Revenue") + ylab("Monthly Revenue ($CAD)") + xlab("Year - Month")
```

### Monthly Profit

The two main costs we examined were fuel costs and instruction costs, since computing them was more accessible given the flight centre's dataset and website pricing information. We computed the profit by subtracting these costs from the revenue, and graphed it by month. Operating a flight centre and offering flight lessons would certainly incur other costs, however we didn't have enough reliable information for these costs. If in the future we acquire such information, we would certainly like to update our profit estimates.
...
cost2 = cost1 %>%
  ...
  filter(Aircraft.x == Aircraft.y,  Aircraft.y == Aircraft)

monthly_profit = cost2 %>%
  mutate(profit = total_revenue_per_month - instructor_cost_per_month - total_monthly_fuel)

monthly_profit %>%
  ggplot(aes(x = Year_Month, y = profit, fill = Aircraft)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Figure 2: Year - Month vs Monthly Profit") +
  ylab("Monthly Profit ($CAD)") +
  xlab("Year - Month")
```

It seems like most of the flight centre's profit came from renting the C-172 planes to students during the months of July, August, and September, even though Instruction Costs and Fuel Costs for C-172 were also higher. This is likely because students spent more time flying C-172 planes and the C-172 planes were more expensive to rent.
...
studentcosts %>% ggplot(aes(x=Student, y=Average_Cost, fill=Cost_Type)) + geom_bar(stat="identity") +
  ggtitle("Figure 3: Costs of Training Types of Students") + 
  ylab("Average Cost of Training ($CAD)")
```
...
```{r echo=FALSE, fig.height=3.25, fig.width=7, message=FALSE, warning=FALSE}
my_cd %>%
  ...
  filter(successful > 0) %>%
  ggplot(aes(x = factor(Student_ID), y = total_time, fill = Aircraft)) + geom_bar(stat = "identity") + 
  theme(axis.text.x = element_blank()) + 
  xlab("Student ID") + 
  ggtitle("Figure 4a: Successful Students vs. Flight Time on C-152, C-172") +
  ylab("Total Flying Time Per Month (hours)")
...
my_cd %>%
  ...
  summarize( successful = sum(solo), total_time = sum(Duration)) %>%
  filter(successful == 0) %>%
  ggplot(aes(x = factor(Student_ID), y = total_time, fill = Aircraft)) + geom_bar(stat = "identity") + theme(axis.text.x = element_blank()) + xlab("Student ID")  + ylab("Total Flying Time Per Month (hours)") + ggtitle("Figure 4b: Unsuccessful Students vs. Flight Time on Aircrafts")
```

### Successful Students Exercises

We were interested in what successful students do that sets them apart from unsuccessful students. On the next page, to the left, we have a plot showing the frequency of Exercises completed by all successful students combined. On the right we have a plot showing the frequency of Exercises completed by all unsuccessful students combined.

We see that the successful students and unsuccessful students completed Exercises 16, 17, 18 and 30 the most often. Exercises 16, 17, 18, and 30 respectively correspond to Take-off, Circuit, Approach and Landing, and Radio Communications. All the successful students combined completed Exercises 16, 17, 18, and 30 1512 times, 1293 times, 1499 times, and 1453 times respectively. All the unsuccessful students combined completed Exercises 16, 17, 18, and 30 412 times, 354 times, 396 times, and 448 times respectively. Successful students practiced these exercises over three times as much as the unsuccessful students. This suggests that Take-off, Circuit, Approach and Landing, and Radio Communications are key components of pilot training. We believe that the number of times a student practices these exercises may be correlated with ability to fly solo. We also noticed that successful students completed Exercise 29 (Emergency Procedures) nearly 5 times as often as unsuccessful students. Successful students also completed Exercises 21-24 (Precautionary Landing, Forced Landing, Pilot Navigation, Instrument Flying) more often than unsuccessful students. Successful students likely have more practice doing different types of landing and navigation than unsuccessful students.
...
student_exercises %>%
  ggplot(aes(x=Exercises_int, y=Exercises_count, fill = Aircraft)) + geom_bar(stat="identity") + facet_grid(.~ Student_type) + xlab("Exercises") + ylab("Number of Times Exercises Were Completed") + ggtitle("Figure 4c: Number of Times Exercises Were Completed")
```

### Progression of Training
We were also interested in finding out what the progression of a successful student’s training looks like. Here is a graph showing the frequency of exercises successful students complete as their training progresses. We found that the average successful student's training lasts 50 days, so we showed the frequency of exercises for the first 50 days of training.
...
my_cd %>% group_by(Student_ID) %>%
  ...
  inner_join(successfulstudents, by = "Student_ID") %>%
  summarize(mean_training_length = mean(Training_Length)) -> mean_training_days
...
training_progression %>%
  filter(Day_Since_First_Session <= mean_training_days) %>%
  ggplot(aes(x=Day_Since_First_Session, y=Exercises_count, fill = ex)) + geom_bar(stat="identity") + ggtitle("Figure 4d: Number of Exercises Completed Over Training") + xlab("Days Since First Session") + ylab("Number of Times Exercises Were Completed")
```

Here we see the progression of training, as successful students mostly focus on familiarisation and preparation for flights (through exercises 1-5) for the first couple of days, and shift to slightly more advanced training with maneuvering and controls (through exercises 6-10) in the next few days. From Day 11 onwards, we observe that exercises 16, 17, 18, 30 are practiced often and continue to be practiced throughout the training. This is indicated by the medium blue color that dominates a large area of the graph. Students may sometimes practice Exercises 20-24, and 29, involving Illusions Created by Drift, Precautionary Landing, Forced Landing, Pilot Navigation, Instrument Flying, and Emergency Procedures, which are more advanced pilot training exercises.

From graph 4c, we see that even though all students practise the more basic exercises (1-10), unsuccessful students practise these in greater frequency. The more prominent difference between the two categories of students, however, stems from the observation that successful students practiced exercises 16, 17, 28, 30 over three times as much as the unsuccessful students. We also observe that the proportion of successful students that use C-152 for completing these exercises is much greater than the proportion of unsuccessful students, who tend to mostly stick with C-172. 

One can infer that successful students display high dedication and consistency in completing Take-off, Circuit, Approach and Landing, and Radio Communications throughout their training. But it would be misleading to conclude that students who shift their focus towards exercises 16, 17, 28, 30 or the ones who train with C-152 are more likely to become successful, because we cannot assume that all students have the same level of experience before they start training at the flight centre. It is possible that some successful students have prior experience in flying, and that is why they spend less time on the more basic exercises. On the other hand, someone with less or no prior experience should focus a great deal on the basic exercises to familiarise themselves with the more basic exercises and getting comfortable with one type of plane. 

We hope that these student profiles of ‘success’ will assist the flight centre in designing training schedules that offer the best value-for-money sessions to the students. Depending on the student’s experience, the flight centre can recommend which exercises are best suited for the student and how many times they should practice before progressing to more advanced ones. They can also recommend which plane is more suited for completing each exercise, while considering the student’s familiarity and preference. 

From the data and graphs, we didn’t observe a ‘hard-and-fast’ rule to achieve success. Even though we don’t have data about the student’s prior experience in training, we can only infer that students who wish to be more successful should simply dedicate more hours in training. The flight centre can use this data to encourage more students to apply more effort and persistence through longer periods of training to become successful.

## Summary

Overall, we found that the flight centre's students log more flight hours during late summer to early fall months, and prefer to do so on the C-172. The centre earns more revenue and profit during these months on C-172 aircrafts, but also spends more on fuel and instruction. We computed fuel and instruction costs of training the average student and compared them with the same costs for the average "successful" student. We explored common traits of successful students and determined that successful ones seemed to stick to one aircraft consistently. We graphed the progression of successful students' training. Exercises 16, 17, 18, and 30 were important in student success. We hope our findings are interesting and relevant to Durham Flight Centre.  

## Appendix

### References
We consulted the following sources to learn how to process certain types of data and format R Markdown files.

https://e2a207d9ab5c4c3d9007aa1ad8e21ccd.app.rstudio.cloud/help/doc/html/packages.html

http://www.stat.cmu.edu/~cshalizi/rmarkdown/

https://stackoverflow.com/questions/29568956/how-to-add-a-rank-column-to-sorted-dates-in-r
...
A limitation of our current analysis is that we assumed the cost of fuel consumed per hour remained constant over 2018-2020 regardless of aircraft model, age and condition, nature of exercise, market conditions for fuel pricing (especially given COVID-19 pandemic), and other factors. We assumed FMX-1000 and Ground training consumed 0 fuel.
...
The graph of instructor cost is in line with the graph of fuel cost. This makes sense because the cost is synonymous with the duration the planes are flown for. 
...
We wanted to estimate the flight centre’s revenue with the ultimate goal of estimating profits. We didn’t know how much the flight centre earned from Ground School alone. On top of this, due to COVID-19 pandemic, the flight centre also stopped offering in-person ground school sessions. Due to this ambiguity, we estimated revenue using the training duration from the flight centre’s provided dataset and the flight centre’s website showing the different pricing options. We assumed these prices were constant throughout 2018-2020.
